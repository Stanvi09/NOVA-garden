        ...
        switch (act.id) {
          ...
          case "impact":
            tl.to(q(".impact-glow"), { opacity: 0.5, r: 260, duration: 1.4, ease: "power2.out" }, t0 + 0.3)
              ...
              .to(q(".root-path"), { stroke: "#6b4fa0", duration: 0.6 }, t0 + 0.8);
            break;
          ...
          case "root_cause":
            ...
            tl.to(q(".world"), { y: -430, duration: 1.2, ease: "power2.inOut" }, t0 + 0.1)
              .to(q(".root-path"), { stroke: "#6b4fa0", duration: 0.4 }, t0 + 1.2)
          ...
          case "seasons":
            tl.to(q(".season-tint"), { opacity: 0.16, duration: 0.4 }, t0 + 0.2)
              .to(q(".season-tint"), { fill: "#f2c675", duration: 0.5 }, t0 + 0.7)
              .to(q(".season-tint"), { fill: "#8a6bb0", duration: 0.5 }, t0 + 1.4)
              .to(q(".season-tint"), { fill: "#6b5090", duration: 0.5 }, t0 + 2.1);
            break;
          ...
          case "resolution":
            tl.to(q(".weed"), { opacity: 0, scale: 0.4, stagger: 0.08, duration: 0.5 }, t0 + 0.3)
              .to(q(".root-path"), { opacity: 0.25, stroke: "#6b4fa0", duration: 0.5 }, t0 + 0.3)
              .to(q(".impact-glow, .risk-ring, .season-tint"), { opacity: 0, duration: 0.4 }, t0 + 0.3)
              .to(q(".plant-leaf"), { fill: "#ff8fc7", duration: 0.6 }, t0 + 0.5)
  ...
  return (
    <div ref={containerRef} className="scroll-stage">
      <svg viewBox="0 0 1200 700" className="stage-svg" role="img" aria-label="Animated illustration of a codebase garden responding to a bug being found, understood, and resolved">
        ...
        <rect className="season-tint" x={0} y={0} width={1200} height={700} fill="#ff8fc7" />
        ...
        <g className="world">
          ...
          <g className="roots-layer">
            ...
            <circle
              ...
              fill="#6b4fa0"
            />
            <text x={PLANT_X[FOCUS_INDEX] - 140} y={935} textAnchor="middle" className="root-path" fontFamily="var(--font-mono)" fontSize={13} fill="#3ddad0">
              Authentication
            </text>
          </g>

          {/* Impact radius glow, centered on the focus plant */}
          <circle className="impact-glow" cx={PLANT_X[FOCUS_INDEX]} cy={620} r={0} fill="#ff5c86" opacity={0} />
              ...
              {[0, 1].map((wi) => (
                <g className="weed" key={wi} opacity={0}>
                  <line
                    ...
                    strokeWidth={2.5}
                  />
                  <circle cx={x - 18 + wi * 14} cy={531} r={4.5} fill="#ff5c86" />
                </g>
              ))}

              {i === FOCUS_INDEX && (
                <g className="sprout-new" opacity={0}>
                  <path d={`M ${x + 34} 560 Q ${x + 40} 540 ${x + 34} 522`} stroke="#3ddad0" strokeWidth={3} fill="none" />
                  <ellipse cx={x + 34} cy={520} rx={8} ry={5} fill="#3ddad0" />
                </g>
              )}
            </g>
          ))}
          ...
          {[PLANT_X[FOCUS_INDEX - 1], PLANT_X[FOCUS_INDEX + 1]].map((x, i) => (
            <g className="whatif-ghost" key={i} opacity={0}>
              <line x1={x} y1={560} x2={x + 4} y2={536} stroke="#ff5c86" strokeWidth={2} strokeDasharray="2 3" />
              <circle cx={x + 4} cy={533} r={4} fill="none" stroke="#ff5c86" strokeWidth={1.5} strokeDasharray="2 2" />
            </g>
          ))}
          ...
          <g className="memory-echo" opacity={0}>
            <line x1={PLANT_X[FOCUS_INDEX] - 60} y1={560} x2={PLANT_X[FOCUS_INDEX] - 56} y2={538} stroke="var(--color-muted)" strokeWidth={2} strokeDasharray="2 3" />
            <circle cx={PLANT_X[FOCUS_INDEX] - 56} cy={535} r={4} fill="none" stroke="var(--color-muted)" strokeWidth={1.5} />
            <text x={PLANT_X[FOCUS_INDEX] - 56} y={520} textAnchor="middle" fontFamily="var(--font-mono)" fontSize={11} fill="var(--color-muted)">
              seen before
            </text>
          </g>
          ...
          <g className="developer-tag" opacity={0}>
            <circle cx={PLANT_X[FOCUS_INDEX] + 70} cy={470} r={16} fill="var(--color-surface)" stroke="#f2c675" strokeWidth={1.5} />
            <text x={PLANT_X[FOCUS_INDEX] + 70} y={475} textAnchor="middle" fontFamily="var(--font-mono)" fontSize={12} fill="#3ddad0">
              CE
            </text>
            <text x={PLANT_X[FOCUS_INDEX] + 70} y={498} textAnchor="middle" fontFamily="var(--font-mono)" fontSize={11} fill="var(--color-muted-strong)">
              Checkout expert
            </text>
          </g>
        </g>
        ...
        <defs>
          <linearGradient id="scanGradient" x1="0" x2="1" y1="0" y2="0">
            <stop offset="0%" stopColor="#ff5c86" stopOpacity="0" />
            <stop offset="50%" stopColor="#ff5c86" stopOpacity="0.35" />
            <stop offset="100%" stopColor="#ff5c86" stopOpacity="0" />
          </linearGradient>
        </defs>
        <text className="scan-label" x={600} y={120} textAnchor="middle" fontFamily="var(--font-mono)" fontSize={22} fill="#ff5c86" opacity={0}>
          BUG DETECTED — Checkout
        </text>

        {/* Autopsy card */}
        <g className="autopsy-card" opacity={0}>
          <rect x={860} y={180} width={260} height={128} rx={10} fill="var(--color-surface)" stroke="var(--color-sage-soft)" />
          <text x={880} y={210} fontFamily="var(--font-mono)" fontSize={11} fill="var(--color-muted)">BUG AUTOPSY</text>
        ...
        <g className="dashboard-mock" opacity={0}>
          <rect x={120} y={90} width={960} height={520} rx={16} fill="var(--color-surface)" stroke="var(--color-sage-soft)" />
          ...
          <text x={166} y={200} fontFamily="var(--font-mono)" fontSize={11} fill="var(--color-muted)">PROJECT HEALTH</text>
          ...
          <text x={386} y={200} fontFamily="var(--font-mono)" fontSize={11} fill="var(--color-muted)">ACTIVE BUGS</text>
          ...
          <text x={606} y={200} fontFamily="var(--font-mono)" fontSize={11} fill="var(--color-muted)">CRITICAL</text>
          <text x={606} y={234} fontFamily="var(--font-display)" fontSize={26} fill="#ff5c86">1</text>
          <rect x={148} y={292} width={824} height={270} rx={8} fill="var(--color-ivory)" stroke="var(--color-sage-soft)" />
          <text x={166} y={318} fontFamily="var(--font-mono)" fontSize={11} fill="var(--color-muted)">LIVE ECOSYSTEM VIEW</text>
        </g>
      </svg>
