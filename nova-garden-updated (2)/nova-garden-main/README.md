...
- **Issue CRUD**: create/list/update issues with automatic history tracking, plus title search and status/severity filtering on the issue list
- **Accessibility**: keyboard-navigable garden plants (each is a focusable, labeled element announcing module name, health level, and score), a labeled modal dialog with Escape-to-close, `aria-live` result counts, and labeled form controls throughout
...
`npm test --prefix backend` runs 24 unit tests (Vitest) covering auth
(signup/login/password hashing), project-role RBAC enforcement, the
module-health scoring algorithm, and the local similarity/analysis
utilities. Tests mock the Prisma client rather than hitting a real
database, so they run fast and deterministically in any environment,
including CI.

## Known limitations / what I'd add with more time

This is a hackathon-scoped vertical slice, not a feature-complete tracker.
Deliberately out of scope for this submission:

- **File attachments on issues** — screenshots/logs would attach well to
  the existing `Issue` model but aren't wired up yet.
- **Full-text search / saved queries** — the issue list currently supports
  title search plus status and severity filters; a saved-query system
  (Bugzilla's signature feature) would be the next addition.
- **Email notifications** — notifications are in-app only (`Notification`
  model); an email/webhook dispatcher would sit behind the same
  `notificationService.ts` calls.
- **Custom fields / keywords / components hierarchy** — modules are a flat
  list per project; nested components and custom issue fields aren't modeled.
- **Frontend test coverage** — backend logic has unit tests; the React
  components don't yet (would add Vitest + React Testing Library next).
- **End-to-end/integration tests** — current tests are unit-level with a
  mocked Prisma client; a real test-database integration suite (e.g. an
  ephemeral SQLite file per test run) would close the gap to the actual
